...
import { AREAS } from '../../../src/models/area';
import type { Area } from '../../../src/models/area';
...
export default function IntegrationsPage() {
	...
	const [todoistCount, setTodoistCount] = useState<number | null>(null);
	...
	function simulateMicrosoftEvents(): { summary: string; start: string; end: string }[] {
		...
		return [
			{
				summary: 'Microsoft: Lunch catch-up',
				start: e1Start.toISOString(),
				end: e1End.toISOString(),
			},
			{
				summary: 'Microsoft: Status review',
				start: e2Start.toISOString(),
				end: e2End.toISOString(),
			},
		];
	}
	...
	// Todoist import (read-only): paste API token to import open tasks
	type TodoistDue = { date?: string; datetime?: string } | null;
	type TodoistTask = {
		id: string;
		content: string;
		description?: string;
	...
	const parseEstimateFromDescription = (description: string | undefined): number | null => {
		if (!description) return null;
		const match = description.match(/(\d{1,3})\s*(m|min|minutes)/i);
		if (match) {
	...
	const onImportTodoist = async () => {
		setTodoistError(null);
		setTodoistCount(null);
		if (!todoistToken.trim()) {
			setTodoistError('API token required');
			return;
		}
		try {
			...
			if (!res.ok) {
				...
				throw new Error(`Failed to fetch tasks (${res.status}): ${text.slice(0, 200)}`);
			}
			...
			for (const t of tasks) {
				...
				const areaLabel = labels.find((l) => (allowedAreas as readonly string[]).includes(l));
				const estimateFromDesc = parseEstimateFromDescription(t.description);
				const priority = Number(t.priority ?? 2);
				const mappedImportance = Math.max(0, Math.min(10, Math.round((priority / 4) * 10)));
				const deadline = (t.due && (t.due.datetime || t.due.date)) || undefined;
				const task: Omit<Task, 'id'> = {
					userId: LOCAL_USER_ID,
					title,
					area: (areaLabel as Area) || 'work',
					estimateMinutes: estimateFromDesc ?? 30,
					importance: mappedImportance,
					energyType: labels.includes('deep') ? 'deep' : 'shallow',
					flexibility: 5,
			...
			setTodoistCount(imported);
		} catch (err: any) {
			setTodoistError(err?.message || 'Import failed');
		}
	};
	...
	const onCsvUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		for (const row of parsed.data as any[]) {
			...
			const areaToken = String(row.area || 'work').toLowerCase();
			const area: Area = (AREAS as readonly string[]).includes(areaToken)
				? (areaToken as Area)
				: 'work';
			const task: Omit<Task, 'id'> = {
				userId: LOCAL_USER_ID,
				...
				energyType:
					String(row.energyType || 'shallow').toLowerCase() === 'deep' ? 'deep' : 'shallow',
	...
	return (
		<main>
			...
			<p style={{ color: 'var(--muted)' }}>
				Import calendars (ICS) and tasks (CSV). Connectors below are read-only stubs.
			</p>
			<section style={{ display: 'grid', gap: 16, maxWidth: 720, marginTop: 16 }}>
				...
				<div>
					...
					{googleCount !== null && (
						<p style={{ color: 'var(--muted)' }}>Imported {googleCount} Google events</p>
					)}
					{msCount !== null && (
						<p style={{ color: 'var(--muted)' }}>Imported {msCount} Microsoft events</p>
					)}
				</div>
				<div>
				...
				<div>
					...
					<div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
						<input
							...
							style={{ minWidth: 260 }}
						/>
						<button onClick={onImportTodoist} disabled={!todoistToken.trim()}>
							Import from Todoist
						</button>
					</div>
					{todoistCount !== null && (
						<p style={{ color: 'var(--muted)' }}>Imported {todoistCount} tasks from Todoist</p>
					)}
					{todoistError && <p style={{ color: 'var(--danger, #e5484d)' }}>{todoistError}</p>}
				</div>
			</section>
		</main>
	);
